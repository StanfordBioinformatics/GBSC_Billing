...
def check_sheets(wkbk):
    ...
    for sheet_name in BILLING_CONFIG_SHEET_COLUMNS:
        ...
        if not wkbk.sheet_loaded(sheet_name):
            print("check_sheets: Can't load %s sheet." % (sheet_name), file=sys.stderr)
            sheets_are_OK = True
        ...
        for header in expected_headers:
            if header not in sheet_headers:
                print("check_sheets: Sheet %s: Expected header %s missing." % (sheet_name, header), file=sys.stderr)
                sheets_are_OK = False
